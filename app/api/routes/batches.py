...
from app.schemas.batches import BatchDetailResponse
from app.schemas.common import BatchListResponse

router = APIRouter()


@router.get("", summary="List batches", response_model=BatchListResponse)
def list_batches(
    ...
    batches = inspector.list_batches(
        status=status,
    ...
    return BatchListResponse(count=len(batches), items=batches)


@router.get("/{batch_id}", summary="Get batch details", response_model=BatchDetailResponse)
def get_batch_details(
    ...
    return BatchDetailResponse(**payload)
